...
# Goal: Create a dictionary that stores the indices (number of the position) of each letter in a word
...
# For the input “grapes”, the output should be:
...
#     if type(letter) != str:
#         continue
#     elif letter not in user_dict.keys():
#         user_dict.update({letter: [index]})
#     else:
...
# Goal: Create a program that prints a list of items that can be purchased with a given amount of
# money.
...
# You will be provided with a dictionary (items_purchase) where the keys are the item names and the
# values are their prices (as strings with a dollar sign).
