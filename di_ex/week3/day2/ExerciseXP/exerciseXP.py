...
# Use the provided Pets and Cat classes to create a Siamese breed, instantiate cat objects, and use
...
# You can add any specific attributes or methods for the Siamese breed, or leave it as is if there
# are no unique behaviors.
...
# cat_1 = Bengal("Timmy", 3)
# cat_2 = Chartreux("Sam", 4)
# cat_3 = Siamese("Jonah", 2)
...
class Dog:
    ...
    def fight(self, other_dog):
        if self.run_speed() > other_dog.run_speed():
            return f"{self.name} won the race"
        elif self.run_speed() < other_dog.run_speed():
            return f"{other_dog.name} won the race"
        else:
...
class Family:
    ...
    def born(self, first_name, age):
        new_person = Person(first_name, age, last_name=self.last_name)
        self.members_list.append(new_person)
    ...
    def family_presentation(self):
        ...
        for item in self.members_list:
            print(f"{item.first_name} is {item.age} years old")
...
# Don’t forget to test your classes by creating an instance of Family, adding members, and calling
# each method to see the expected output.

my_family = Family("Miron")
my_family.born("Ehud", 58)
my_family.born("Tova", 62)
my_family.born("Hadas", 30)
my_family.born("Ohad", 28)
my_family.born("Uri", 15)
my_family.check_majority("Uri")
