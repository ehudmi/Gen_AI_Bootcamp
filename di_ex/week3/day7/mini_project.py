...
# It has the following attribute : humans, the list containing the humans that are waiting. It is
# initialized empty.
...
# get_next(self) : Returns the next human waiting in the queue. The next human should be the one
# located at the index 0 in the list.
...
# Bonus: Don’t use any of the following built-in methods: list.insert, list.pop, list.index,
# list.sort, sorted.
...
# Initialized as empty, family is a list of all the humans that are living in the same house with
# this human.
# Add a method add_family_member(self, person) that adds the person to this human’s family and this
# human to the person’s family.


# Queue
# Add the rearrange_queue(self) method to the Queue class, so that there won’t be two members of
# the same family one after the other in the line.
